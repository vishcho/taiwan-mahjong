...
**V0.1 修正（2026-09-12）：** 以下公式保留為拆解與交叉驗證基準，但不能獨立處理「需要第五張同牌」的情況。反例 `123456m789p123s1111z` 為 16 張；四組面子加四張東可讓公式得到 0，卻沒有任何合法摸牌能直接胡牌。第一版改以「到合法五面子一眼目標的最少缺牌數 − 1」作為對外向聽數，此例為 1。目標牌型與自己的固定副露合計，每種牌不得超過四張；對手副露、牌河等公開牌仍只影響實際受入，不改變結構向聽數。Runtime 使用 Base-5 距離表與 DP，獨立的目標牌型 DP 交叉驗證。拆解畫面若引用原公式的代表解，必須顯示差異。
